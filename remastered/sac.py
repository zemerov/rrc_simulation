...
class SAC(object):
    def __init__(self, num_inputs, action_space, args, env):
        ...
        if self.policy_type == "Gaussian":
            ...
            self.policy = GaussianPolicy(num_inputs, action_space, args.hidden_size, env).to(self.device)
            self.policy_optim = Adam(self.policy.parameters(), lr=args.lr)

        else:
            self.alpha = 0
            self.automatic_entropy_tuning = False
            self.policy = DeterministicPolicy(num_inputs, action_space, args.hidden_size, env).to(self.device)
            self.policy_optim = Adam(self.policy.parameters(), lr=args.lr)
    ...
    def update_parameters(self, memory, batch_size, updates):
        ...
        state_batch, action_batch, reward_batch, next_state_batch, done = memory.sample(batch_size=batch_size)
